 ...
 */
export template <typename T> using Result = std::expected<T, Error>;
...
auto Ok(T &&value) -> std::expected<std::remove_reference_t<T>, Error> {
    return std::expected<std::remove_reference_t<T>, Error>(
        std::forward<T>(value));
}
 ...
 *
 * This function is used to create successful results for operations that don't
 * return a value.
 */
export inline auto Ok() -> std::expected<void, Error> { return {}; }
 ...
 */
export template <typename T> auto unwrap(const Result<T> &result) -> T {
    return *result;
}
