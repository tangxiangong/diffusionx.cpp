...
public:
    ...
    /**
     * @brief Simulates the birth-death process and interpolates to regular time grid
     * @param duration The total simulation time
    ...
    [[nodiscard]] auto theoretical_variance(double t) const -> double {
        if (m_birth_rate == m_death_rate) {
            return static_cast<double>(m_initial_state) * (m_birth_rate + m_death_rate) * t;
        } else {
            double r = m_birth_rate - m_death_rate;
            double exp_rt = std::exp(r * t);
            return static_cast<double>(m_initial_state) * (m_birth_rate + m_death_rate) / r * 
                   exp_rt * (exp_rt - 1.0);
        }
    }
}; 