...
export class BirthDeathProcess {
  ...
  public:
     ...
     */
    BirthDeathProcess(double birth_rate, double death_rate,
                      int initial_state = 1)
    ...
    Result<vec_pair> simulate_with_steps(size_t num_steps) {
        if (num_steps == 0) {
            return Err(
                Error::InvalidArgument("Number of steps must be positive"));
        }
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) {
        ...
        // Interpolate to regular time grid using piecewise constant
        // interpolation
        ...
        for (size_t i = 0; i <= num_steps; ++i) {
            ...
            // Find the state at this time using piecewise constant
            // interpolation
            auto it = std::upper_bound(event_times.begin(), event_times.end(),
                                       times[i]);
