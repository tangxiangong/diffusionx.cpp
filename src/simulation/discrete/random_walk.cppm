...
import diffusionx.simulation.basic.discrete;
 ...
 * 
 * where ξ_i are i.i.d. random variables taking values ±1 with probability 1/2 each.
 */
...
public:
     ...
     */
    [[nodiscard]] auto get_step_size() const -> double {
        return m_step_size;
    }
    ...
    Result<discrete_pair> simulate(size_t num_steps) override {
        if (num_steps == 0) {
            return Ok(std::make_pair(vector<size_t>{0}, vector<double>{m_start_position}));
        }
...
public:
     ...
     * @param start_position Initial position
     * @param probability Probability of moving in positive direction (must be in [0, 1])
     * @param step_size Size of each step (must be positive)
     * @throws std::invalid_argument if parameters are invalid
     */
    BiasedRandomWalk(double start_position, double probability, double step_size = 1.0)
        : m_start_position(start_position), m_step_size(step_size), m_probability(probability) {
     ...
     */
    [[nodiscard]] auto get_probability() const -> double {
     ...
     */
    [[nodiscard]] auto theoretical_variance(size_t n) const -> double {
        return 4.0 * n * m_probability * (1.0 - m_probability) * m_step_size * m_step_size;
    }
};
 ...
 * 
 * This class implements a random walk with step sizes drawn from a stable distribution,
 * generalizing the simple random walk to heavy-tailed step distributions.
 */
...
public:
     ...
     * @throws std::invalid_argument if parameters are invalid
     */
    LevyRandomWalk(double alpha, double beta, double sigma, double start_position = 0.0)
        : m_alpha(alpha), m_beta(beta), m_sigma(sigma), m_start_position(start_position) {
        if (m_alpha <= 0.0 || m_alpha > 2.0) {
            throw std::invalid_argument("Stability parameter alpha must be in (0, 2]");
        }
    ...
    Result<discrete_pair> simulate(size_t num_steps) override {
        ...
        auto increments_result = rand_stable(num_steps, m_alpha, m_beta, m_sigma, 0.0);
    ...
    [[nodiscard]] auto theoretical_mean(size_t n) const -> double {
        if (m_alpha > 1.0) {
            return m_start_position;  // Mean of stable distribution is 0 for our parameterization
        }
