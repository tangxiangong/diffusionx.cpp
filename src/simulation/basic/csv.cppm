 ...
 */
export Result<int> write_trajectory_csv(const string &filename,
                                        const vector<double> &times,
                                        const vector<double> &positions) {
    if (times.size() != positions.size()) {
        return Err(Error::InvalidArgument(
            "Times and positions vectors must have the same size"));
    }

    std::ofstream file(filename);
    if (!file.is_open()) {
        return Err(
            Error::IoError("Failed to open file for writing: " + filename));
    }
 ...
 */
export Result<int>
write_multiple_trajectories_csv(const string &filename,
    ...
    for (size_t traj_id = 0; traj_id < trajectories.size(); ++traj_id) {
        ...
        if (times.size() != positions.size()) {
            return Err(
                Error::InvalidArgument("Times and positions vectors must have "
                                       "the same size for trajectory " +
 ...
 * @return Result indicating success or an Error
 */
export Result<int> write_tamsd_csv(const string &filename,
                                   const vector<double> &lag_times,
                                   const vector<double> &tamsd_values) {
    if (lag_times.size() != tamsd_values.size()) {
        return Err(Error::InvalidArgument(
            "Lag times and TAMSD values vectors must have the same size"));
    }
 ...
 * @return Result indicating success or an Error
 */
export Result<int> write_msd_csv(const string &filename,
                                 const vector<double> &times,
                                 const vector<double> &msd_values) {
    if (times.size() != msd_values.size()) {
        return Err(Error::InvalidArgument(
            "Times and MSD values vectors must have the same size"));
    }
...
export Result<vec_pair> read_trajectory_csv(const string &filename) {
    ...
    if (!file.is_open()) {
        return Err(
            Error::IoError("Failed to open file for reading: " + filename));
    }
