 ...
 *
 * This module provides functions for computing the time-averaged mean squared
 * displacement, which is a key quantity for analyzing anomalous diffusion and
 * ergodicity breaking.
 */
...
export Result<double> tamsd(const vector<double> &trajectory, size_t lag_time) {
    ...
    if (lag_time >= trajectory.size()) {
        return Err(Error::InvalidArgument(
            "Lag time must be less than trajectory length"));
    }
 ...
 */
export Result<vector<double>> tamsd_multiple(const vector<double> &trajectory,
                                             size_t max_lag_time) {
    if (trajectory.empty()) {
        return Err(Error::InvalidArgument("Trajectory cannot be empty"));
    }
    ...
    if (max_lag_time >= trajectory.size()) {
        return Err(Error::InvalidArgument(
            "Maximum lag time must be less than trajectory length"));
    }
 ...
 */
export Result<double> ensemble_tamsd(const vector<vector<double>> &trajectories,
                                     size_t lag_time) {
    if (trajectories.empty()) {
        return Err(
            Error::InvalidArgument("Trajectories vector cannot be empty"));
    }
    ...
    if (valid_count == 0) {
        return Err(Error::InvalidArgument(
            "No valid trajectories for TAMSD computation"));
    }
 ...
 * @param trajectories Vector of trajectory data
 * @param lag_time The lag time for computing the TAMSD
 * @return Result containing a vector of TAMSD values from each trajectory, or
 * an Error
 */
export Result<vector<double>>
tamsd_distribution(const vector<vector<double>> &trajectories,
                   size_t lag_time) {
    if (trajectories.empty()) {
        return Err(
            Error::InvalidArgument("Trajectories vector cannot be empty"));
 ...
 * EB = <δ²>² / <δ⁴> - 1
 *
 * where <δ²> is the ensemble average of TAMSD and <δ⁴> is the ensemble average
 * of TAMSD². For ergodic processes, EB ≈ 0. For non-ergodic processes, EB > 0.
 */
export Result<double>
ergodicity_breaking_parameter(const vector<vector<double>> &trajectories,
                              size_t lag_time) {
    ...
    if (tamsd_values.size() < 2) {
        return Err(Error::InvalidArgument(
            "Need at least 2 valid trajectories for EB computation"));
    }
