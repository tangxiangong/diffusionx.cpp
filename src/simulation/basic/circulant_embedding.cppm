 ...
 *
 * The circulant embedding method is used for exact simulation of Gaussian
 * processes with stationary covariance functions. It requires that the
 * circulant matrix constructed from the covariance function is positive
 * semidefinite.
 */
export Result<vector<double>>
circulant_embedding(size_t n,
 ...
 */
export Result<vector<double>> ou_circulant_embedding(size_t n, double theta,
                                                     double sigma, double dt) {
    if (theta <= 0.0) {
        return Err(Error::InvalidArgument(
            "Mean reversion parameter must be positive"));
    }
    if (sigma <= 0.0) {
        return Err(
            Error::InvalidArgument("Volatility parameter must be positive"));
    }
    if (dt <= 0.0) {
