 ...
 * 
 * This module provides the basic infrastructure for simulating functional distribution.
 * 
 * - First passage time
 * - Occupation time
 * 
 ...
 * 
 * This class represents the first passage time of a stochastic process through a given domain.
 * The first passage time is defined as the first time the process exits the interval (a, b).
 * 
...
public:
     ...
     * This constructor should only be used when the object will be assigned later.
     */
     ...
     * 
     * @throws std::invalid_argument if the domain is not a valid interval (a >= b)
     * 
     ...
     * ```
     */
    FirstPassageTime(T &process, double_pair domain) : m_process{process}, m_domain{domain} {
        auto [a, b] = domain;
        if (a >= b) {
            throw std::invalid_argument(format(
                "The domain (a, b) must be a valid interval, but got ({}, {})", a, b));
        }
    }
 ...
 * 
 * This class represents the occupation time of a stochastic process within a given domain
 * over a specified duration. The occupation time measures how long the process spends
 * within the interval (a, b) during the time period [0, T].
 * 
 * @tparam T The type of the stochastic process
 * 
 * Mathematical definition:
...
public:
     ...
     * 
     * @throws std::invalid_argument if the domain is not a valid interval (a >= b)
     * @throws std::invalid_argument if the duration is not positive (duration < 0)
     * 
     * @pre The domain must satisfy a < b
     ...
     * ```
     */
    OccupationTime(T &process, double_pair domain, double duration) : m_process{process}, m_domain{domain},
                                                                      m_duration{duration} {
        auto [a, b] = domain;
        if (a >= b) {
            throw std::invalid_argument(format(
        ...
        if (duration < 0) {
            throw std::invalid_argument(format(
                "The `duration` must be positive, but got {}", duration
            ));
        }
    }
