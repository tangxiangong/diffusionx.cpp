...
public:
    ...
    BrownianBridge(double start_value, double end_value, double total_time)
        : m_start_value(start_value), m_end_value(end_value), m_total_time(total_time) {
        if (m_total_time <= 0) {
     ...
     */
    [[nodiscard]] auto get_start_value() const -> double {
     ...
     */
    [[nodiscard]] auto get_end_value() const -> double {
        return m_end_value;
    }
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        if (std::abs(duration - m_total_time) > 1e-10) {
            return Err(Error::InvalidArgument("Duration must equal total_time for Brownian bridge"));
        }
     ...
     */
    [[nodiscard]] auto theoretical_covariance(double s, double t) const -> double {
...
private:
     ...
     */
    Result<void> generate_bridge_recursive(vector<double>& positions, const vector<double>& times,
        ...
        auto left_result = generate_bridge_recursive(positions, times, left_idx, mid_idx);
        if (!left_result.has_value()) {
            return Err(left_result.error());
        }

        auto right_result = generate_bridge_recursive(positions, times, mid_idx, right_idx);
