...
export class BrownianBridge final : public ContinuousProcess {
  ...
  public:
    ...
    Result<vec_pair> simulate(double duration, double time_step) override {
        ...
        if (auto result =
                generate_bridge_recursive(positions, times, 0, num_steps);
            !result.has_value()) {
  ...
  private:
    /**
     * @brief Recursively generates Brownian bridge using sequential
     * conditioning
    ...
    static Result<void> generate_bridge_recursive(vector<double> &positions,
        ...
        double variance =
            (t_mid - t_left) * (t_right - t_mid) / (t_right - t_left);
