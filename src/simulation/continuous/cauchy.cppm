...
public:
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        auto increments_result = rand_stable(num_steps, 1.0, 0.0, scaled_sigma, 0.0);
     ...
     * @return False for all orders ≥ 1 (Cauchy distribution has no finite moments)
     */
...
public:
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        auto increments_result = rand_stable(num_steps, 1.0, m_beta, scaled_sigma, 0.0);
        if (!increments_result.has_value()) {
            return Err(increments_result.error());
        }
        auto increments = increments_result.value();
