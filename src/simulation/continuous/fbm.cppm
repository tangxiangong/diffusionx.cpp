...
export class FBm final : public ContinuousProcess {
  ...
  private:
     ...
     */
    static Result<vector<double>>
    generate_fbm_circulant_embedding(size_t n, double hurst) {
        ...
        for (size_t i = 0; i < m; ++i) {
            ...
            eigenvalues[i] =
                std::max(0.0, eigenvalues[i]); // Ensure non-negative
        }
        ...
        for (size_t i = 0; i < m; ++i) {
            z[i] =
                complex<double>(std::sqrt(eigenvalues[i]) * gaussian[i], 0.0);
        }
