...
public:
     ...
     */
    [[nodiscard]] auto get_hurst() const -> double {
        return m_hurst;
    }
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 1; i <= n; ++i) {
            times[i] = i * time_step;
            positions[i] = m_start_position + fbm_increments[i - 1] * std::pow(time_step, m_hurst);
        }
...
private:
     ...
     */
    Result<vector<double>> generate_fbm_circulant_embedding(size_t n, double hurst) {
        ...
        fftw_plan plan = fftw_plan_dft_1d(static_cast<int>(m), in, out, FFTW_FORWARD, FFTW_ESTIMATE);
        ...
        for (size_t i = 0; i < m; ++i) {
            ...
            if (eigenvalues[i] < -1e-10) {  // Allow small numerical errors
                ...
                fftw_free(out);
                return Err(Error::InvalidArgument("Circulant matrix is not positive semidefinite"));
            }
        ...
        fftw_plan inv_plan = fftw_plan_dft_1d(static_cast<int>(m), z_in, z_out, FFTW_BACKWARD, FFTW_ESTIMATE);
