...
class Langevin : public ContinuousProcess {
    ...
    static_assert(std::is_invocable_r_v<double, DiffusionFunc, double, double>, 
...
public:
     ...
     */
    Langevin(DriftFunc drift_func, DiffusionFunc diffusion_func, double start_position = 0.0)
...
public:
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 1; i <= num_steps; ++i) {
            ...
            positions[i] = x + m_drift_func(x, t) * ds + 
 ...
 */
export template<typename DriftFunc, typename DiffusionFunc>
auto make_langevin(DriftFunc&& drift_func, DiffusionFunc&& diffusion_func, double start_position = 0.0) {
    return Langevin<std::decay_t<DriftFunc>, std::decay_t<DiffusionFunc>>(
        std::forward<DriftFunc>(drift_func), 
        std::forward<DiffusionFunc>(diffusion_func), 
        start_position
    );
}
 ...
 */
export template<typename DriftFunc, typename DiffusionFunc>
auto make_generalized_langevin(DriftFunc&& drift_func, DiffusionFunc&& diffusion_func, 
                              double start_position = 0.0, double alpha = 1.5) {
    return GeneralizedLangevin<std::decay_t<DriftFunc>, std::decay_t<DiffusionFunc>>(
        std::forward<DriftFunc>(drift_func), 
        std::forward<DiffusionFunc>(diffusion_func), 
 ...
 */
export template<typename DriftFunc, typename DiffusionFunc>
auto make_subordinated_langevin(DriftFunc&& drift_func, DiffusionFunc&& diffusion_func, 
                               double start_position = 0.0, double alpha = 0.5) {
    return SubordinatedLangevin<std::decay_t<DriftFunc>, std::decay_t<DiffusionFunc>>(
        std::forward<DriftFunc>(drift_func), 
        std::forward<DiffusionFunc>(diffusion_func), 
        start_position, alpha
    );
