 ...
 * 
 * This class implements the Ornstein-Uhlenbeck process, a mean-reverting stochastic process
 * commonly used to model interest rates, volatility, and other financial quantities.
 * 
...
public:
     ...
     */
    OrnsteinUhlenbeck(double theta, double mu, double sigma, double start_position)
        : m_theta(theta), m_mu(mu), m_sigma(sigma), m_start_position(start_position) {
        if (m_theta <= 0) {
            throw std::invalid_argument("Mean reversion speed theta must be positive");
        }
        if (m_sigma <= 0) {
     ...
     */
    [[nodiscard]] auto get_theta() const -> double {
        return m_theta;
    }
     ...
     * X(t + dt) = X(t) * exp(-θ * dt) + μ * (1 - exp(-θ * dt)) + σ * √((1 - exp(-2θ * dt)) / (2θ)) * Z
     * where Z ~ N(0, 1)
     */
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        double var_coeff = m_sigma * std::sqrt((1.0 - std::exp(-2.0 * m_theta * time_step)) / (2.0 * m_theta));
        ...
        for (size_t i = 1; i <= num_steps; ++i) {
            times[i] = i * time_step;
            positions[i] = positions[i - 1] * exp_theta_dt + m_mu * mean_coeff + var_coeff * increments[i - 1];
        }
    ...
    [[nodiscard]] auto theoretical_variance(double t) const -> double {
        return (m_sigma * m_sigma / (2.0 * m_theta)) * (1.0 - std::exp(-2.0 * m_theta * t));
    }
}; 