...
public:
     ...
     */
    Levy(double alpha, double beta, double sigma, double mu, double start_position = 0.0)
        : m_alpha(alpha), m_beta(beta), m_sigma(sigma), m_mu(mu), m_start_position(start_position) {
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        auto increments_result = rand_stable(num_steps, m_alpha, m_beta, scaled_sigma, scaled_mu);
