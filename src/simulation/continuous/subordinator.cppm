...
public:
     ...
     * 
     * Uses the fact that increments follow a maximally skewed stable distribution:
     * S(t + dt) - S(t) ~ Stable(α, 1, dt^(1/α), 0)
     */
...
public:
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 1; i <= num_steps; ++i) {
            ...
            while (sub_index < sub_positions.size() && sub_positions[sub_index] <= target_level) {
                ++sub_index;
            }
            ...
            if (sub_index < sub_times.size()) {
                ...
                positions[i] = positions[i - 1] + time_step / std::pow(target_level, m_alpha);
            }
        }
