 ...
 * (mean square displacement ~ t) but has non-Gaussian displacement distributions.
 * This is achieved by coupling a diffusion process with a time-varying
 * diffusion coefficient driven by an Ornstein-Uhlenbeck process.
 * 
...
public:
    ...
    BrownianNonGaussian(double start_position, double ou_start_position)
        : m_start_position(start_position), m_ou_start_position(ou_start_position) {
    }
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 1; i <= num_steps; ++i) {
            ...
            double increment = std::sqrt(2.0 * diffusion_coeff * time_step) * noise[i - 1];
...
private:
     ...
     */
    Result<vector<double>> simulate_ou_process(double duration, double time_step) {
        size_t num_steps = static_cast<size_t>(std::ceil(duration / time_step));
        ...
        double var_coeff = sigma * std::sqrt((1.0 - std::exp(-2.0 * theta * time_step)) / (2.0 * theta));
        ...
        for (size_t i = 1; i <= num_steps; ++i) {
            y_trajectory[i] = y_trajectory[i - 1] * exp_theta_dt + var_coeff * ou_noise[i - 1];
        }
