 ...
 * 
 * This class implements the standard Brownian motion, a continuous-time stochastic process
 * with independent, normally distributed increments. The process satisfies:
 * - B(0) = start_position
...
public:
     ...
     * 
     * Creates a Brownian motion starting at position 0 with unit diffusion coefficient.
     */
    ...
    BrownianMotion(double start_position, double diffusion_coefficient)
        : m_start_position(start_position), m_diffusion_coefficient(diffusion_coefficient) {
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        // Generate increments
        auto increments_result = randn(num_steps, 0.0, std::sqrt(2.0 * m_diffusion_coefficient * time_step));
    ...
    Result<Option<double>> fpt(double_pair domain, double max_duration = 1000,
        ...
        if (lower >= upper) {
            return Err(Error::InvalidArgument("Invalid domain: lower bound must be less than upper bound"));
        }
