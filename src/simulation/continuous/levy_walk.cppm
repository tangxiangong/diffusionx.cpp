...
public:
     ...
     * @return The velocity
     */
    [[nodiscard]] auto get_velocity() const -> double {
        return m_velocity;
    }
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 0; i <= num_steps; ++i) {
            times[i] = i * time_step;
            positions[i] = interpolate_position(times[i], event_times, event_positions);
        }
...
private:
    /**
     * @brief Interpolates position at given time using piecewise linear interpolation
     * @param t Target time
...
public:
    ...
    Result<vec_pair> simulate(double duration, double time_step = 0.01) override {
        ...
        for (size_t i = 0; i <= num_steps; ++i) {
            times[i] = i * time_step;
            positions[i] = interpolate_position_constant(times[i], event_times, event_positions);
        }
...
private:
     ...
     * @return Interpolated position
     */
    double interpolate_position_constant(double t, const vector<double>& event_times, 
                                        const vector<double>& event_positions) const {
        if (t <= event_times[0]) {
            return event_positions[0];
        }
