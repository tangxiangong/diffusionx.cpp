...
export class LevyWalk final : public ContinuousProcess {
  ...
  public:
    ...
    LevyWalk(double alpha, double velocity, double start_position = 0.0)
        : m_alpha(alpha), m_velocity(velocity),
  ...
  private:
     ...
     */
    static double interpolate_position(double t,
                                       const vector<double> &event_times,
        ...
        for (size_t i = 1; i < event_times.size(); ++i) {
            if (t <= event_times[i]) {
                // Linear interpolation between event_times[i-1] and
                // event_times[i]
...
export class CoupledCTRW final : public ContinuousProcess {
  ...
  public:
    ...
    Result<vec_pair> simulate(double duration, double time_step) override {
        ...
        for (size_t i = 0; i <= num_steps; ++i) {
            times[i] = i * time_step;
            positions[i] = interpolate_position_constant(times[i], event_times,
                                                         event_positions);
        }
